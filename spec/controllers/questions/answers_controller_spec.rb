  ...
  describe '非同期通信によるcreateアクション' do
    context "フォーム入力が無効であった場合" do
      it "回答の作成に失敗する" do
        login_user(user)
        expect do
        post :create, xhr: true, params: { answer_form: { post_id: test_post.id, body: "" } }
    ...
    context "フォーム入力が有効であった場合" do
      ...
      it "回答の作成に成功する" do
        login_user(user)
        expect do
        post :create, xhr: true, params: { answer_form: { post_id: test_post.id, body: answer_form.body } }
  ...
  describe '同期通信によるcreateアクション' do
    context "フォーム入力が無効であった場合" do
      it "回答の作成に失敗する" do
        login_user(user)
        expect do
        post :create, params: { answer_form: { post_id: test_post.id, body: "" } }
        end.not_to change(Answer, :count)
        expect(response).to have_http_status "200"
    ...
    context "フォーム入力が有効であった場合" do
      let(:answer_form) { build(:answer_form, post_id: test_post.id) }
      it "回答の作成に成功する" do
        login_user(user)
        expect do
        post :create, params: { answer_form: { post_id: test_post.id, body: answer_form.body } }
        end.to change(Answer, :count).by(1)
        expect(response).to have_http_status "302"
        expect(response).to redirect_to questions_post_path(answer_form.post_id)
      end
    end
  end
