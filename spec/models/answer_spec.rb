  ...
  describe 'send_notice_to_questioner_or_answerersメソッド' do
    context '自己解決でない場合' do
      it '質問者の通知カラムがtrueを返す' do
        create(:question_notice, user_id: questioner.id, post_id: post.id) # 質問者の通知レコードを作成
        answer = create(:answer, user_id: answerer1.id, post_id: post.id) # 質問者以外の人が回答を作成
        answer.send_notice_to_questioner_or_answerers
        question_notice = QuestionNotice.find_by(user_id: questioner, post_id: post)
        expect(question_notice.notice).to eq true
      end
    end
    context '自己解決の場合' do
      it '回答者の通知カラムがtrueを返す' do
        answer_entry1 = create(:question_notice, user_id: answerer1.id, post_id: post.id) # 回答者の通知レコードを作成
        answer_entry2 = create(:question_notice, user_id: answerer2.id, post_id: post.id)
