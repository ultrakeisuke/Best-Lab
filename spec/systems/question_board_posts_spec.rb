...
RSpec.describe '質問の作成と編集', type: :system do
  ...
  it '質問の新規作成におけるカテゴリーの動的変化', js: true do
    ...
    expect(find("#parent_category")).to have_content "#{parent_category1.name}" # 親カテゴリーの初期表示
    expect(find("#children_category")).to have_content "#{parent1_children_categories[0].name}" # 子カテゴリーの初期表示
    # 親カテゴリーのセレクトボックスをクリックすると他の親カテゴリーも表示される
    find("#parent_category").click
    expect(find("#parent_category")).to have_content "#{parent_category2.name}"
    # 他の親カテゴリーをクリックすると子カテゴリーの表示が変わる
    find("#parent_category").find("option:nth-child(2)").select_option
    expect(find("#children_category")).not_to have_content "#{parent1_children_categories[0].name}" # 初期表示の子カテゴリーが非表示になる
    expect(find("#children_category")).to have_content "#{parent2_children_categories[0].name}" # 他の子カテゴリーが表示される
  end
  ...
  it '質問の編集におけるカテゴリーの動的変化', js: true do
    ...
    expect(find("#parent_category")).to have_content "#{parent_category2.name}" # 投稿の親カテゴリーを表示
    expect(find("#children_category")).to have_content "#{parent2_children_categories[1].name}" # 投稿の子カテゴリーを表示
    # 親カテゴリーのセレクトボックスをクリックすると他の親カテゴリーも表示される
    find("#parent_category").click
    expect(find("#parent_category")).to have_content "#{parent_category1.name}"
    # 他の親カテゴリーをクリックすると子カテゴリーの表示が変わる
    find("#parent_category").find("option:nth-child(1)").select_option
    expect(find("#children_category")).not_to have_content "#{parent2_children_categories[1].name}" # 初期表示の子カテゴリーが非表示になる
    expect(find("#children_category")).to have_content "#{parent1_children_categories[0].name}" # 他の子カテゴリーが表示される
  end
