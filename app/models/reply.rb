class Reply < ApplicationRecord
  ...
  def send_notice_to_questioner_and_answerer
    ...
    if user != answer.user # 回答者でない場合は回答者に通知を送信
      answerer = QuestionNotice.find_by(user_id: answer.user_id, post_id: post_id)
  ...
  def send_notice_to_commenter
    ...
    repliers = QuestionNotice.where(user_id: replier_ids, post_id: post_id).where.not(user_id: user_id)
